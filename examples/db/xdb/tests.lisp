(defpackage :xdb/tests
  (:use :cl :rt :obj/db :obj/id :xdb :obj/meta/storable))

(in-package :xdb/tests)
(defsuite :xdb)
(in-suite :xdb)

(defparameter *tree* nil)

(defclass test-doc-non-storable (id)
  ((eid :initarg :eid)
   (aa :initarg :aa)
   (bb :initarg :bb)
   (cc :initarg :cc)
...
;;; DB
(deftest db ()
  "Test database protocol."
  (format t "Hash Test~%")
  (format t "Store~%")
  (time (test-store-doc-hash col-hash 10000))
  (format t "Sum~%")
  (time (xdb::sum col-hash :element "id"))
  (format t "Find~%")
  (time (xdb::find-doc col-hash :test (lambda (doc) (equal (get-val doc "id") 500))))
  (format t "Sort~%")
  (time (xdb::sort-collection col-hash))
  (format t "List Test~%")
  (format t "Store~%")
  (time (test-store-doc-list col-list 10000))
  (format t "Sum~%")
  (time (xdb::sum col-list :element "id"))
  (format t "Find~%")
  (time (xdb::find-doc col-list :test (lambda (doc) (equal (get-val doc "id") 500))))
  (format t "Sort~%")
  (time (xdb::sort-collection col-list))
  ...
  (format t "Sum~%")
  (time (xdb::sum col-object :element 'id))
  (format t "Find~%")
  (time (xdb::find-doc col-object :test (lambda (doc) (equal (get-val doc 'id) 500))))
  (format t "Sort~%")
  (time (xdb::sort-collection col-object))


  (setf xdb::*fsync-data* nil)
  (format t "Object storable Test~%")
  (format t "Store~%")
  (time (test-store-doc-storable-object col-object-storable 10000))
  (format t "Sum~%")
  (time (xdb::sum col-object-storable :element 'id))
  (format t "Find~%")
  (time (xdb::find-doc col-object-storable :test (lambda (doc) (equal (get-val doc 'id) 500))))
  (format t "Sort~%")
  (time (xdb::sort-collection col-object-storable)))
