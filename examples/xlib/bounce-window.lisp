;;; bounce-window.lisp --- Bounce window

;; BOUNCE-WINDOW takes a window and seemingly drops it to the bottom of the
;; screen.  Optionally, the window can have an initial x velocity, screen
;; border elasticity, and gravity value.  The outer loop is entered the first
;; time with the window at its initial height, but each iteration after this,
;; the loop starts with the window at the bottom of the screen heading upward.
;; The inner loop, except for the first execution, carries the window up until
;; the negative velocity becomes positive, carrying the window down to bottom
;; when the velocity is positive.  Due to number lossage, ROUND'ing and
;; TRUNC'ing when the velocity gets so small will cause the window to head
;; upward with the same velocity over two iterations which will cause the
;; window to bounce forever, so we have prev-neg-velocity and number-problems
;; to check for this.  This is not crucial with the x velocity since the loop
;; terminates as a function of the y velocity.
