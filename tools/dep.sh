...
get_architecture() {
    ...
    if [ "${_ostype}" = unknown-linux-gnu ] && [ "${_bitness}" -eq 32 ]; then
        case $_cputype in
            x86_64)
                if [ -n "${BABEL_CPUTYPE:-}" ]; then
                    _cputype="$BABEL_CPUTYPE"
                else {
                    ...
                    if is_host_amd64_elf; then {
                         ...
                         echo "by re-running this script with the BABEL_CPUTYPE environment variable" 1>&2
...
main () {
    ...
    # no extension unless on windows
    local _ext=""
    case "$_arch" in
        *windows*)
            _ext=".exe"
  ...
  local _url="${PKG_URL_ROOT}/bin/dist/${_arch}/${PKG_NAME}${_ext}"

    local _dir
    _dir="$(ensure mktemp -d)"
    local _file="${_dir}/${PKG_NAME}${_ext}"
    ...
    if [ "$1" = --check ]; then
        ...
        if [ -n "$_err" ]; then
            ...
            if echo "$_err" | grep -q 404$; then
                err "installer for platform '$3' not found 8^C - ask ellis to support your platform"
            fi
        fi
        return $_status
    elif [ "$_dld" = wget ]; then
        ...
        if [ -n "$_err" ]; then
            ...
            if echo "$_err" | grep -q ' 404 Not Found$'; then
                err "installer for platform '$3' not found!"
            fi
        fi
        return $_status
    else
        err "Unknown dl program"   # should not reach here
    fi
}
...
get_ciphersuites_for_curl() {
    if [ -n "${BABEL_TLS_CIPHERSUITES-}" ]; then
        # user specified custom cipher suites, assume they know what they're doing
        RETVAL="$BABEL_TLS_CIPHERSUITES"
        return
    fi
